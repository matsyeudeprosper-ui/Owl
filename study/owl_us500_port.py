...
R = mt5.copy_rates_from_pos("US500m", mt5.TIMEFRAME_M1, 1, 99000)
mt5.shutdown()
    ...
    def step(self, i):
        ...
        if up.get("pending"):
            ...
            if T[i] - up.get("rt_t", T[i]) > 21600:
                up["retest"] = None
            elif (cl <= up["retest"] + 0.55
                  and cc >= up["retest"] - 0.55):
        ...
        if dn.get("pending") and sig is None:
            ...
            if T[i] - dn.get("rt_t", T[i]) > 21600:
                dn["retest"] = None
            elif (ch >= dn["retest"] - 0.55
                  and cc <= dn["retest"] + 0.55):
...
class Machine:
    ...
    def open_pos(self, i, d, lot, e, wall, chainpos):
        ...
        tpd = tpd * 0.25   # CONTROL: fixed short TP
