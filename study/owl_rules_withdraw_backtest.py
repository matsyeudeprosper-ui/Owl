...
import sys
KEEP = float(sys.argv[1]) if len(sys.argv) > 1 else 157.0
print(f"=== WITHDRAWAL sim, keep-level ${KEEP:.0f} ===", flush=True)
...
for i in range(N):
    ...
    if positions:
        ...
        if eq < 8.0 * len(positions):
            ...
            trades.append((tm[i], cash, 'account_death'))
            deposits += START_BAL
            cash = START_BAL
            pending_wd = False
            positions.clear(); groups.clear()

    # --- weekly withdrawal: only when FLAT, down to keep-level ---
    if next_wd is None:
        next_wd = tm[i] + 604800
    if tm[i] >= next_wd:
        pending_wd = True
        next_wd += 604800
    if pending_wd and not positions and cash > KEEP:
        pocket += cash - KEEP
        cash = KEEP
        wd_count += 1
        pending_wd = False
...
if deaths_l:
    ...
    print(f"last death: {death_dates[-1] if death_dates else None}")
    prev = tm[0]
    lens = []
    for t in deaths_l:
...
net = pocket + cash - deposits
print(f"WITHDRAWALS: {wd_count} times, pocketed ${pocket:,.2f}")
print(f"OVERALL NET (pocket + account - deposits): ${net:,.2f}  (${net/((tm[-1]-tm[0])/86400/30.44):,.2f}/mo)")
