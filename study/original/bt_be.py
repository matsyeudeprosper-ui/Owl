"""Breakeven test: once a trade travels 70% of the way to TP, the
SL jumps to entry (same-bar re-check, pessimistic). Versus
deployed. Also 50% and 85% arming for the curve."""
...
def run(a, b, bek):
    ...
    for j in range(a, b):
        ...
        if pos is not None:
            d, e, sl, tp, armed = pos
            fav = (h - e) if d == 1 else (e - (l + S))
            if bek and not armed and fav >= bek * abs(tp - e):
                armed = True
                sl = max(sl, e) if d == 1 else min(sl, e + S)
                pos = (d, e, sl, tp, armed)
...
for lbl, k in (("deployed (no BE)", 0),
               ("BE at 50%", 0.5),
               ("BE at 70%", 0.7),
               ("BE at 85%", 0.85)):
    n, cnt, w, dd = run(0, len(T), k)
    na, _, _, _ = run(0, mid, k)
    nb, _, _, _ = run(mid, len(T), k)
    print(f"{lbl:16s}: net {n:+8.2f} n {cnt:4d} "
