"""FULL SYSTEM simulation - the deployed machine exactly: awake
gate + touch/flip entries + war-chest ledger (debt, chest cap $5,
start empty) + fighters at entry (up to +3 bullets when in debt,
chest-guaranteed) + 50%-pullback adds (up to +2 bullets, chest-
guaranteed) + live bookkeeping rules. Versus the flat-0.02 config.
"""
...
def run(a, b, fighters=True, adds=True):
    st = Struct()
    pos = None
    ev = deque()
    out = []
    ...
    def book(pnl, lot, is_add):
        ...
        if is_add:
            ...
            return
        if pnl < 0:
            base_sh = pnl * min(1.0, BASE / max(lot, 0.01))
            debt += -base_sh
...
for lbl, f, ad in (("flat 0.02 (no chest)", False, False),
                   ("fighters only", True, False),
                   ("adds only", False, True),
                   ("FULL (deployed)", True, True)):
    n, cnt, w, dd, nf, na2 = run(0, len(T), f, ad)
    na_, _, _, _, _, _ = run(0, mid, f, ad)
    nb_, _, _, _, _, _ = run(mid, len(T), f, ad)
    print(f"{lbl:20s}: net {n:+8.2f} n {cnt:4d} "
          f"wr {w/max(1,cnt):.0%} maxDD {dd:6.2f} "
          f"fights {nf:3d} adds {na2:3d} | "
