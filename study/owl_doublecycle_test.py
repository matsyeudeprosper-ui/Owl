...
class Machine:
    ...
    def settle(self):
        net = -self.chain_loss       # positive = the cycle won
        self.chain_loss = 0.0
        if net < -0.5:
            self.debt += -net
            if self.cyc_scaled:
                self.bank = 0.0      # scaled loss spends the bank
        elif net > 0 and self.debt > 0.5:
            self.bank += net
            self.debt = max(0.0, self.debt - net)
            if self.debt <= 0.5:
                self.debt = 0.0
                self.bank = 0.0
        self.cyc_scaled = False
    ...
    def close(self, i, px, kind):
        ...
        if kind in ("sl", "scratch"):
            ...
            if not p["chain"]:
                ...
                self.settle()        # the ONE fighter lost: cycle over
        else:
            if pnl > 0:
                self.won = True
            self.chain_loss -= pnl
    ...
    def bar(self, i):
        ...
        if w is not None:
            ...
            nl = min(0.04, round(w["lot"] + 0.01, 2))
            if broke:
        ...
        if sig is not None:
            ...
            dist = abs(e - wall)
            lot = PAGE_LOT
            if self.debt > 0.5 and dist >= MIN_WALL:
                for Lx in (0.04, 0.03, 0.02):
                    if (self.bank >= 0.5 * dist * Lx
                            and dist * Lx <= LINK_CAP):
                        lot = Lx
                        break
            if lot != PAGE_LOT:
                self.chain_loss = 0.0
                self.cyc_scaled = True
                if not self.virtual:
                    SCALED.append(lot)
                self.open_pos(i, d, lot, e, wall, True)
            elif MIN_WALL <= dist and dist * PAGE_LOT <= PAGE_MAXR:
                self.chain_loss = 0.0
...
print('scaled recovery cycles fired:', len(SCALED), SCALED[:25])
print('debt left on book: %.2f  bank: %.2f' % (real.debt, real.bank))