...
class Machine:
    ...
    def __init__(self, virtual):
        ...
        self.won = False
        self.chest = 0.0
        self.next_lot = 0.02     # ladder position, cap 0.04
    ...
    def bar(self, i):
        ...
        if sig is not None:
            sig = None
        if _rnd() < 1.0 / 470.0:
            _d0 = 1 if _rnd() < 0.5 else -1
            _w0 = (C[i] - 80.0) if _d0 == 1 else (C[i] + 80.0)
            sig = (_d0, _w0)
        if f is not None:
            if T[i] - f["t"] > 21600:
                self.flip = None
