...
breakeven-plus, partial85 (>=0.02), deep >=1.12 bank70 + heal(debt+3
...
SPREAD, MIN_WALL, LINK_CAP, CHAIN_CAP = None, 2.1, 35.27, 100.0
PAGE_LOT, PAGE_TGT, PAGE_MAXR, BUFFER = 0.28, 1.50, 2.50, 0.10
mt5.initialize(path=r"C:\Projects\MT5-KinoliveTrader\terminal64.exe")
R = mt5.copy_rates_from_pos("ETHUSDm", mt5.TIMEFRAME_M1, 1, 99000)
mt5.shutdown()
...
T = R["time"].astype(int)
SPREAD = float(_np0.median(R["spread"])) * 0.01
N = len(R)

import numpy as _np
_tr = _np.maximum(H[1:] - L[1:], _np.maximum(
    _np.abs(H[1:] - C[:-1]), _np.abs(L[1:] - C[:-1])))
_cs = _np.concatenate([[0.0], _np.cumsum(_tr)])
def _atr14(i):
    a, b = max(0, i - 15), i - 1
    ...
    def step(self, i):
        ...
        if up.get("pending"):
            ...
            if T[i] - up["pt"] > 21600:
                ...
                if cc - up["pending"] > max(0.7, 0.35 * wd):
            ...
            if T[i] - up.get("rt_t", T[i]) > 21600:
                up["retest"] = None
            elif (cl <= up["retest"] + 0.53
                  and cc >= up["retest"] - 0.53):
        ...
        if dn.get("pending") and sig is None:
            ...
            if T[i] - dn["pt"] > 21600:
                ...
                if dn["pending"] - cc > max(0.7, 0.35 * wd):
            ...
            if T[i] - dn.get("rt_t", T[i]) > 21600:
                dn["retest"] = None
            elif (ch >= dn["retest"] - 0.53
                  and cc <= dn["retest"] + 0.53):
...
class Machine:
    ...
    def __init__(self, virtual):
        ...
        self.won = False
        self.chest = 0.0
        self.next_lot = 0.56     # ladder position, cap 1.12
    ...
    def open_pos(self, i, d, lot, e, wall, chainpos):
        ...
        tpd = dist - disc / lot
        if chainpos and lot >= 1.12 and self.chain_loss > 0:
        ...
        if dist > 0.88:
            _slw = e - d * 0.88
        self.pos = dict(d=d, lot=lot, e=e, sl=_slw, tp=e + d * tpd,
    ...
    def close(self, i, px, kind):
        ...
        self.pos = None
        if not self.virtual and "stg" in p:
            TALLY[p["stg"]].append(pnl)
        if not self.virtual:
            if pnl > 0.05:
                WINS_R.append(pnl)
                del WINS_R[:-25]
        ...
        self.det = Det()      # clean chart after every close (user)
        if kind in ("sl", "scratch"):
            self.chain_loss += max(0.0, -pnl)
            if p["chain"] and pnl < -0.5:
                # REAL loss spends the chest + steps the ladder;
                # breakeven exits change nothing (user 09-06)
                self.chest = 0.0
                self.next_lot = min(1.12,
                                    round(p["lot"] + 0.28, 2))
            else:
                pass                     # losses do not drain the pot
        else:
            if pnl > 0:
                self.won = True
            if p["chain"]:
                # user 09-06: a win pays only what it won; recovery
                # continues until the whole book is empty
                self.chain_loss = max(0.0, self.chain_loss - pnl)
                self.chest = 0.0
                self.next_lot = 0.56
            elif self.chain_loss > 0.5:
                self.chest = min(5.0, self.chest + pnl)
            else:
                self.chain_loss = 0.0
                if pnl > 0:
                    self.chest = min(5.0, self.chest + pnl)
    ...
    def bar(self, i):
        ...
        if p is not None:
            ...
            if (not p["parted"] and p["lot"] >= 0.56
                    and fav >= 0.85 * prize):
                out.append((0.85 * prize * (p["lot"] / 2), "partial"))
                p["parted"] = True
                p["lot"] = round(p["lot"] / 2, 2)
            if p["chain"] and p["lot"] >= 1.12 and fav >= 0.70 * prize:
        ...
        if sig is not None:
            ...
            dist = abs(e - wall)
            if self.chain_loss > 0.5:
                risk = dist * self.next_lot
                if (self.chest >= risk and dist >= MIN_WALL
                        and risk <= LINK_CAP):
                    FIGHTS.append(self.next_lot); self.open_pos(i, d, self.next_lot, e, wall, True)
                elif (MIN_WALL <= dist
                      and dist * PAGE_LOT <= PAGE_MAXR):
                    _stg = abs(C[i] - O[i]) >= _atr14(i)
                    if SKIP_CALM and _stg:
                        return out
...
print('fighters fired:', len(FIGHTS))
for _k, _lbl in ((True, "STRONG"), (False, "calm ")):
    _v = TALLY[_k]
    if _v:
        _w = sum(1 for x in _v if x > 0.005)
        print(f"{_lbl}: {len(_v)} trades, {_w} wins "
              f"({100*_w/len(_v):.0f}%), net {sum(_v):+.2f}")
